...
SECRETS_FILE="vault/secrets.file"
...
case "${1}" in
	(''|help|--help)
		...
		*** don't use quotes and equal signs in passwords, please ***
		...
		;;
	(init)
		git config filter.encryptionmagic.smudge './vault/secrets.sh decrypt'
		git config filter.encryptionmagic.clean './vault/secrets.sh encrypt'
